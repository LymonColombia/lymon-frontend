...
@Component({
  ...
  imports: [TenantPageLayoutComponent, ButtonComponent, InputComponent, SelectComponent, NgIcon],
  providers: [
    provideIcons({
      bootstrapPeopleFill,
      bootstrapPerson,
  ...
  templateUrl: './guests-crm.html',
  styleUrl: './guests-crm.css',
})
export class GuestsCrmComponent implements OnInit {
  ...
  navigateToFullProfile(guest: CrmGuest): void {
    const guestId = this.getGuestRouteId(guest);
    if (!guestId) return;
    void this.router.navigate(['/admin/crm/guests', guestId]);
  }
